...
def pwnagotchi_cli():
        ...
        while True:
            try:
                ...
                if str(e).find("wifi.interface not set") > 0:
                    logging.exception(
                        "main loop exception due to unavailable wifi device, likely programmatically disabled (%s)",
                        e,
                    )
                    logging.info(
    ...
    if args.wizard:
        ...
        if pwn_restore in ("y", "yes"):
            os.system(
                "cp -f /etc/pwnagotchi/config.toml.bak /etc/pwnagotchi/config.toml"
            )
            print(
                "Your previous configuration is restored, and I will restart in 5 seconds."
            )
            time.sleep(5)
            ...
            if pwn_check.lower() in ("y", "yes"):
                os.system(
                    "mv -f /etc/pwnagotchi/config.toml /etc/pwnagotchi/config.toml.bak"
                )
                with open("/etc/pwnagotchi/config.toml", "a+") as f:
                    ...
                    if pwn_bluetooth.lower() in ("y", "yes"):
                        if pwn_bluetooth_device.lower == "android":
                            print(
                                "To visit the webui when connected with your phone, visit: http://192.168.44.44:8080\n"
                            ...
                            print(
                                "To visit the webui when connected with your phone, visit: http://172.20.10.6:8080\n"
                                "Your configuration is done, and I will restart in 5 seconds."
                            )
                    else:
                        print(
    ...
    if args.check_update:
        ...
        if remote > local:
            user_input = input(
                f"There is a new version available! Update from v{pwnagotchi.__version__} to v{latest_ver}?\n[Y/N] "
            )
            # input validation
            ...
            print(
                f"You are currently on the latest release, v{pwnagotchi.__version__}."
            )
        sys.exit(0)
