...
class Agent(Client, Automata, AsyncAdvertiser):
    ...
    def _fetch_stats(self):
        while True:
            try:
                ...
                logging.error(f"[agent:_fetch_stats] self.session: {err!r}")

            try:
                self._update_uptime(s)
            except Exception as err:
                logging.error(f"[agent:_fetch_stats] self.update_uptimes: {err!r}")

            try:
                self._update_advertisement(s)
            except Exception as err:
                logging.error(
                    f"[agent:_fetch_stats] self.update_advertisements: {err!r}"
                )

            try:
                self._update_peers()
            except Exception as err:
                logging.error(f"[agent:_fetch_stats] self.update_peers: {err!r}")
            try:
                self._update_counters()
            except Exception as err:
                logging.error(f"[agent:_fetch_stats] self.update_counters: {err!r}")
            try:
                self._update_handshakes(0)
            except Exception as err:
                logging.error(f"[agent:_fetch_stats] self.update_handshakes: {err!r}")
    ...
    async def _on_event(self, msg):
        ...
        try:
            plugins.on(
                f"bcap_{re.sub(r'[^a-z0-9_]+', '_', jmsg['tag'].lower())}", self, jmsg
            )
        except Exception as err:
            logging.error(f"Processing event: {err}")
