...
def run_once(pqueue, event_name, *args, **kwargs):
    try:
        prctl.set_name(f"R1_{pqueue.plugin_name}_{event_name}")
        pqueue.process_event(event_name, *args, *kwargs)
        logging.debug(f"Thread for {pqueue.plugin_name} {event_name} exiting")
    except Exception:
        logging.exception(
            f"Thread for {pqueue.plugin_name}, {event_name}, {args!r}, {kwargs!r}"
        )
...
class PluginEventQueue(threading.Thread):
    def __init__(self, plugin_name):
        try:
            ...
            logging.debug(
                f"PLUGIN EVENT QUEUE FOR {plugin_name} starting {self.load_handler!r}"
            )
            self.start()
    ...
    def run(self):
        logging.debug(f"Worker thread starting for {self.plugin_name}")
        prctl.set_name(f"PLG {self.plugin_name}")
        self.process_events()
        logging.info(f"Worker thread exiting for {self.plugin_name}")

    def process_event(self, event_name, *args, **kwargs):
        cb_name = f"on_{event_name}"
        callback = getattr(loaded[self.plugin_name], cb_name, None)
        logging.debug(f"{self.plugin_name}.{event_name}: {args!r}")
        if callback:
...
class Plugin:
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        plugin_instance = cls()
        logging.debug(f"loaded plugin {plugin_name} as {plugin_instance}")
        ...
        for attr_name in plugin_instance.__dir__():
            if attr_name.startswith("on_"):
                ...
                if cb is not None and callable(cb):
                    locks[f"{plugin_name}::{attr_name}"] = threading.Lock()
...
def on(event_name, *args, **kwargs):
    ...
    for plugin_name in loaded.keys():
        ...
        logging.debug(f"{plugin_name} {cb_name}")
...
def load_from_path(path, enabled=()):
    global loaded, database
    logging.debug(f"loading plugins from {path} - enabled: {enabled}")
    for filename in glob.glob(os.path.join(path, "*.py")):
        ...
        if plugin_name in enabled:
            try:
                ...
                logging.warning(f"error while loading {filename}: {e}")
...
def load(config):
    try:
        ...
        # load custom ones
        custom_path = (
            config["main"]["custom_plugins"]
            if "custom_plugins" in config["main"]
            else None
        )
