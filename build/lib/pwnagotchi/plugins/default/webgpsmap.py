                ...
                if path == "/" or not path:
                    ...
                    try:
                        ...
                        html_data = html_data.replace(
                            ...
                            "var offlinePositions = " + json_data,
                        )
...
class PositionFile:
    ...
    def mac(self):
        ...
        parsed_mac = re.search(
            r".*_?([a-zA-Z0-9]{12})\.(?:gps|geo)\.json", self._filename
        )
        if parsed_mac:
    ...
    def timestamp_first(self):
        ...
        return int(f"{os.path.getctime(self._file):.0f}")
    ...
    def timestamp_last(self):
        ...
        if "ts" in self._json:
            ...
            return_ts = int(f"{dateObj.timestamp():.0f}")
        else:
            # use file timestamp last modification of the json file
            return_ts = int(f"{os.path.getmtime(self._file):.0f}")
        return return_ts
