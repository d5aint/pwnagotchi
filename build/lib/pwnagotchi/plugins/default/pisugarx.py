...
class PiSugarServer:
        ...
        while True:
            try:
                ...
                if self.model == "PiSugar3":
                    ...
                    self.power_plugged = (
                        ctr1 & (1 << 7)
                    ) != 0  # Check if power is plugged in
                    self.allow_charging = (
                        ctr1 & (1 << 6)
                    ) != 0  # Check if charging is allowed
                    if self.max_charge_voltage_protection:
                        self._bus.write_byte_data(
                            self.address, 0x0B, 0x29
                        )  # Disable write protection
                        self._bus.write_byte_data(
                            self.address,
                            0x20,
                            self._bus.read_byte_data(self.address, 0x20) | 0b10000000,
                        )
                        self._bus.write_byte_data(
                            self.address, 0x0B, 0x00
                        ...
                        self._bus.write_byte_data(
                            self.address,
                            0x20,
                            self._bus.read_byte_data(self.address, 0x20) & 0b01111111,
                        )
                        self._bus.write_byte_data(
                            self.address, 0x0B, 0x00
                        )  # Enable write protection
                    ...
                    self.battery_voltage = (
                        (2600.0 - (((high | 0b11000000) << 8) + low) * 0.26855) / 1000.0
                        if high & 0x20
                        else (2600.0 + (((high & 0x1F) << 8) + low) * 0.26855) / 1000.0
                    )
    ...
    def shutdown(self):
        ...
        if self.model == "PiSugar3":
            ...
            self._bus.write_byte_data(
                self.address,
                0x02,
                self._bus.read_byte_data(self.address, 0x02) & 0b11011111,
            )
            self._bus.write_byte_data(
                self.address, 0x0B, 0x00
            )  # Enable write protection
    ...
    def device_init(self):
        ...
        if self.model == "PiSugar2Plus":
            ...
            self._bus.write_byte_data(
                self.address,
                0x52,
                self._bus.read_byte_data(self.address, 0x52) | 0b00000010,
            )
            self._bus.write_byte_data(
                self.address,
                0x54,
                self._bus.read_byte_data(self.address, 0x54) | 0b00000010,
            )
            self._bus.write_byte_data(
                self.address,
                0x52,
                self._bus.read_byte_data(self.address, 0x52) | 0b00000100,
            )
            self._bus.write_byte_data(
                self.address,
                0x29,
                self._bus.read_byte_data(self.address, 0x29) & 0b10111111,
            )
            self._bus.write_byte_data(
                self.address,
                0x52,
                self._bus.read_byte_data(self.address, 0x52) & 0b10011111 | 0b01000000,
            )
            self._bus.write_byte_data(
                self.address,
                0xC2,
                self._bus.read_byte_data(self.address, 0xC2) | 0b00010000,
            )
            logging.debug("PiSugar2Plus GPIO initialization complete")
            """Init boost intensity, 0x3f*50ma, 3A"""
            self._bus.write_byte_data(
                self.address,
                0x30,
                self._bus.read_byte_data(self.address, 0x30) & 0b11000000 | 0x3F,
            )
            logging.debug("PiSugar2Plus current setting complete")
            ...
            self._bus.write_byte_data(
                self.address,
                0x51,
                (self._bus.read_byte_data(self.address, 0x51) & 0b11110011)
                | 0b00000100,
            )
            self._bus.write_byte_data(
                self.address,
                0x53,
                self._bus.read_byte_data(self.address, 0x53) | 0b00000010,
            )
            self._bus.write_byte_data(
                self.address,
                0x51,
                (self._bus.read_byte_data(self.address, 0x51) & 0b11001111)
                | 0b00010000,
            )
            self._bus.write_byte_data(
                self.address,
                0x26,
                self._bus.read_byte_data(self.address, 0x26) & 0b10110000,
            )
            self._bus.write_byte_data(
                self.address,
                0x52,
                (self._bus.read_byte_data(self.address, 0x52) & 0b11110011)
                | 0b00000100,
            )
            self._bus.write_byte_data(
                self.address,
                0x53,
                (self._bus.read_byte_data(self.address, 0x53) & 0b11101111)
                | 0b00010000,
            )
            logging.debug("PiSugar2 GPIO initialization complete")
        pass
    ...
    def convert_battery_voltage_to_level(self):
        ...
        if len(self.voltage_history) < 5:
            ...
            avg_voltage = sum(trimmed_history) / len(
                trimmed_history
    ...
    def set_battery_allow_charging(self):
        if self.model == "PiSugar3":
            ...
            self._bus.write_byte_data(
                self.address,
                0x54,
                self._bus.read_byte_data(self.address, 0x54) & 0b11111011,
            )
            # Enable charging
            self._bus.write_byte_data(
                self.address,
                0x55,
                self._bus.read_byte_data(self.address, 0x55) & 0b11111011,
            )
            # Enable gpio2 output
            self._bus.write_byte_data(
                self.address,
                0x54,
                self._bus.read_byte_data(self.address, 0x54) | 0b00000100,
            )
        elif self.model == "PiSugar2Plus":
            # Disable gpio2 output
            self._bus.write_byte_data(
                self.address,
                0x56,
                self._bus.read_byte_data(self.address, 0x56) & 0b11111011,
            )
            # Enable charging
            self._bus.write_byte_data(
                self.address,
                0x58,
                self._bus.read_byte_data(self.address, 0x58) & 0b11111011,
            )
            # Enable gpio2 output
            self._bus.write_byte_data(
                self.address,
                0x56,
                self._bus.read_byte_data(self.address, 0x56) | 0b00000100,
            )
    ...
    def set_battery_notallow_charging(self):
        if self.model == "PiSugar3":
            ...
            self._bus.write_byte_data(
                self.address,
                0x55,
                self._bus.read_byte_data(self.address, 0x55) | 0b00000100,
            )
            # Enable gpio2 output
            self._bus.write_byte_data(
                self.address,
            ...
            self._bus.write_byte_data(
                self.address,
                0x58,
                self._bus.read_byte_data(self.address, 0x58) | 0b00000100,
            )
            # Enable gpio2 output
            self._bus.write_byte_data(
                self.address,
...
class PiSugar(plugins.Plugin):
    ...
    def on_loaded(self):
        ...
        self.ps.max_charge_voltage_protection = self.options[
            "max_charge_voltage_protection"
        ]
        ...
        try:
            if request.method == "GET":
                if path == "/" or not path:
                    ...
                    battery_safe_shutdown_level = (
                        f"{safe_shutdown_level}%"
                        if safe_shutdown_level is not None
                        else "Not set"
                    )
                    ...
                    battery_soft_poweroff = (
                        self.safe_get(self.ps.get_battery_soft_poweroff, default=False)
                        if model == "Pisugar 3"
                        else False
                    )
                    ...
                    rtc_adjust_ppm = (
                        self.safe_get(
                            self.ps.get_rtc_adjust_ppm, default="Not supported"
                        )
                        if model == "Pisugar 3"
                        else "Not supported"
                    )
                    ...
                    anti_mistouch = (
                        self.safe_get(self.ps.get_anti_mistouch, default=False)
                        if model == "Pisugar 3"
                        else False
                    )
                ...
                try:
                    ...
                    logging.debug(f"[PiSugarX] error during POST: {e!r}")
                    return render_template_string(ret), 500
        except Exception as e:
            ret = "<html><head><title>PiSugarX error</title></head>"
            ret += f"<body><h1>{e!r}</h1></body></html>"
            logging.debug(f"[PiSugarX] error: {e!r}")
    ...
    def on_ui_update(self, ui):
        ...
        if self.ready:
            ...
            voltage = 0.00
            temp = 0
            logging.info("[PiSugarX] PiSugar is not ready")
