...
class UploadConvertPlugin(Plugin):
    ...
    def __init__(self):
        ...
        self.potfile_url = (
            "http://pwncrack.org/download_potfile_script"  # Leave this as is
        )
        self.timewait = 600
    ...
    def on_internet_available(self, agent):
        ...
        if remaining_wait_time > 0:
            logging.debug(
                f"[pwncrack] Waiting {remaining_wait_time:.1f} more seconds before next run."
            )
            return
        self.last_run_time = current_time
        logging.info(
            f"[pwncrack] Running upload process. Key: {self.key}, waiting: {self.timewait} seconds."
        )
        try:
            ...
            logging.error(
                f"[pwncrack] Error occurred during upload process: {e}", exc_info=True
            )
    ...
    def _convert_and_upload(self):
        ...
        if pcap_files:
            ...
            logging.info(
                "[pwncrack] No .pcap files found to convert (or all files are whitelisted)."
            )
    ...
    def _download_potfile(self):
        ...
        if response.status_code == 200:
            ...
            logging.error(
                f"[pwncrack] Failed to download potfile: {response.status_code}"
            )
            logging.error(
                f"[pwncrack] {response.json()}"
