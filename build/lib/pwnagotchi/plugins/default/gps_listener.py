...
while true; do
  ...
  NMEA_GGA=rrrr"\$${RAW_NMEA_GGA}*${CHECKSUM}"
  ...
  NMEA_VTG=rrrr"\$${RAW_NMEA_VTG}*${CHECKSUM_VTG}"
    ...
    def create_virtual_serial_ports(self):
        self.socat_process = subprocess.Popen(
            [
                ...
                "-d",
                "-d",
                f"pty,link={self.write_virtual_serial},mode=777",
    ...
    def run_socat(self):
        while not self.stop_event.is_set():
            self.socat_process = subprocess.Popen(
                [
                    "socat",
                    f"UDP-RECVFROM:{self.listen_port},reuseaddr,bind={self.listen_ip}",
                    "-",
                ],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
