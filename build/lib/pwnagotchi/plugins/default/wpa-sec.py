...
class WpaSec(plugins.Plugin):
    ...
    def on_internet_available(self, agent):
        ...
        with self.lock:
            ...
            try:
                ...
                cursor.execute(
                    "SELECT path FROM handshakes WHERE status = ?",
                ...
                if handshakes_toupload:
                    logging.info(
                        "WPA_SEC: Internet connectivity detected. Uploading new handshakes..."
                    )
                    for idx, handshake in enumerate(handshakes_toupload):
                        display.on_uploading(
                            f"WPA-SEC ({idx + 1}/{len(handshakes_toupload)})"
                        )
                        ...
                        try:
                            ...
                            if upload_response.startswith("hcxpcapngtool"):
                                logging.info(
                                    f"WPA_SEC: {handshake} successfully uploaded."
                                )
                                new_status = self.Status.SUCCESSFULL.value
                            else:
                                logging.info(
                                    f"WPA_SEC: {handshake} uploaded, but it was invalid."
                                )
                            ...
                            logging.exception(
                                "WPA_SEC: RequestException uploading %s, skipping until reload.",
                                handshake,
                            )
                            self.skip_until_reload.append(handshake)
                        except OSError:
                            logging.exception(
                                "WPA_SEC: OSError uploading %s, deleting from db.",
                                handshake,
                            )
                            cursor.execute(
                                "DELETE FROM handshakes WHERE path = ?", (handshake,)
                            )
                            db_conn.commit()
                        except Exception:
                            logging.exception(
                                "WPA_SEC: Exception uploading %s.", handshake
                            )
            ...
            try:
                if (
                    ...
                    cracked_file_path = os.path.join(
                        handshake_dir, "wpa-sec.cracked.potfile"
                    )

                    if os.path.exists(cracked_file_path):
                        last_check = datetime.fromtimestamp(
                            os.path.getmtime(cracked_file_path)
                        )
                        download_interval = int(
                            self.options.get("download_interval", 3600)
                        )
                        if (
                    ...
                    if "single_files" in self.options and self.options["single_files"]:
                        self._write_cracked_single_files(
                            cracked_file_path, handshake_dir
                        )
            except Exception:
        ...
        with open(cracked_file_path, "r") as cracked_file:
            for line in cracked_file:
                try:
                    ...
                    if password:
                        handshake_filename = (
                            re.sub(r"[^a-zA-Z0-9]", "", ssid) + "_" + bssid
                        )
                        pcap_path = os.path.join(
                            handshake_dir, handshake_filename + ".pcap"
                        )
                        pcap_cracked_path = os.path.join(
                            handshake_dir, handshake_filename + ".pcap.cracked"
                        )
                        if os.path.exists(pcap_path) and not os.path.exists(
                            pcap_cracked_path
                        ):
                    ...
                    logging.exception(
                        f"WPA_SEC: Exception writing cracked single file, parsing line {line}."
                    )
    ...
    def on_ui_setup(self, ui):
        if (
            "show_pwd" in self.options
            and self.options["show_pwd"]
            and "download_results" in self.options
            and self.options["download_results"]
        ):
                ...
                with open(file_path, "r") as file:
                    ...
                    if lines:  # Check if file is not empty
                        ...
                        parts = last_line.split(
                            ":"
                        )  # Split line into fields using ':' as a delimiter
                        if len(parts) >= 4:
