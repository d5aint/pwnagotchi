...
class ohcapi(plugins.Plugin):
    ...
    def __init__(self):
        ...
        try:
            self.report = StatusFile(
                "/root/handshakes/.ohc_uploads", data_format="json"
            )
    ...
    def on_ui_update(self, ui):
        ...
        if (
            self.internet_active
            and current_time - self.last_run >= self.options["sleep"]
        ):
    ...
    def _extract_essid_bssid_from_hash(self, hash_line):
        ...
        if essid == "unknown_ESSID" or bssid == "00:00:00:00:00:00":
            logging.debug(
                f"OHC NewAPI: Failed to extract ESSID/BSSID from hash -> {hash_line}"
            )
    ...
    def _run_tasks(self, agent):
        ...
        with self.lock:
            ...
            processed_stations = self.report.data_field_or(
                "processed_stations", default=[]
            )
            ...
            handshake_paths = [
                ...
                if not os.path.exists(p.replace(".pcap", ".22000"))
            ]
            ...
            if handshake_new:
                logging.info(
                    f"OHC NewAPI: Processing {len(handshake_new)} new PCAP handshakes."
                )
                ...
                if all_hashes:
                    batches = [
                        all_hashes[i : i + 50] for i in range(0, len(all_hashes), 50)
                    ]
                    upload_success = True
                    ...
                    if upload_success:
                        ...
                        logging.debug(
                            "OHC NewAPI: Successfully reported all new handshakes."
                        )
                    else:
                        ...
                        for pcap_path in successfully_extracted:
                            self.skip.append(pcap_path)
                        logging.debug(
                            "OHC NewAPI: Failed to upload tasks, added to skip list."
                        )
                else:
                    logging.debug(
