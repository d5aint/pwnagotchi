...
class FixServices(plugins.Plugin):
    ...
    def on_ready(self, agent):
        ...
        try:
            ...
            logging.debug(f"[Fix_Services ip link show wlan0mon]: {cmd_output!r}")
            if ",UP," in str(cmd_output):
                logging.debug("wlan0mon is up.")

        except Exception as err:
            logging.error(f"[Fix_Services ip link show wlan0mon]: {err!r}")
            try:
                self._tryTurningItOffAndOnAgain(agent)
            except Exception as err:
                logging.error(f"[Fix_Services OffNOn]: {err!r}")
    ...
    def on_bcap_sys_log(self, agent, event):
        ...
        if re.search("wifi error while hopping to channel", event["data"]["Message"]):
            logging.debug(f"[Fix_Services]SYSLOG MATCH: {event['data']['Message']}")
            ...
            try:
                ...
                if result["success"]:
                    ...
                    logging.warning(
                        f"[Fix_Services] wifi.recon flip: FAILED: {result!r}"
                    )
                    self._tryTurningItOffAndOnAgain(agent)
            except Exception as err:
                logging.error(f"[Fix_Services]SYSLOG wifi.recon flip fail: {err}")
                self._tryTurningItOffAndOnAgain(agent)
    ...
    def on_epoch(self, agent, epoch, epoch_data):
        ...
        if time.time() - self.LASTTRY > 180:
            ...
            # Look for pattern 2
                ...
                logging.debug(
                    f"[Fix_Services]**** Should trigger a reload of the wlan0mon device:\n{last_lines}"
                )
                if hasattr(agent, "view"):
                    display.set("status", "Wifi channel stuck. Restarting recon.")
                    display.update(force=True)
                ...
                try:
                    ...
                    logging.error(f"[Fix_Services wifi.recon flip] {err!r}")
            ...
            # Look for pattern 3
                ...
                try:
                    ...
                    logging.debug(f"[Fix_Services monstart]: {cmd_output!r}")
                except Exception as err:
                    logging.error(f"[Fix_Services monstart]: {err!r}")
    ...
    def logPrintView(self, level, message, ui=None, displayData=None, force=True):
        try:
            ...
            logging.error(f"[logPrintView] ERROR {err!r}")
        ...
        if self.isReloadingMon and (time.time() - self.LASTTRY) < 180:
            ...
            try:
                ...
                if "success" in result:
                    self.logPrintView(
                        "info",
                        f"[Fix_Services] wifi.recon off: {result!r}!",
                        display,
                    ...
                    self.logPrintView(
                        "warning",
                        f"[Fix_Services] wifi.recon off: FAILED: {result!r}",
                        display,
                        {
                ...
                logging.error(f"[Fix_Services wifi.recon off] error  {err!r}")
            ...
            try:
                ...
                self.logPrintView(
                    "info",
                    f"[Fix_Services] wlan0mon down and deleted: {cmd_output}",
                    display,
                    {"status": "wlan0mon d-d-d-down!", "face": faces.BORED},
                )
            except Exception as nope:
                logging.error(f"[Fix_Services delete wlan0mon] {nope}")
                pass
            ...
            while tries < 3:
                try:
                    ...
                    self.logPrintView(
                        ...
                        display,
                        {"status": f"Turning it off #{tries}", "face": faces.SMART},
                    )
                        ...
                        try:
                            ...
                            try:
                                ...
                                if "success" in result:
                                    ...
                                    logging.debug(
                                        f"[Fix_Services set wifi.interfaceface wlan0mon] failed? {result!r}"
                                    )
                            except Exception as err:
                                logging.debug(
                                    f"[Fix_Services set wifi.interface wlan0mon] except: {err!r}"
                                )
                        except Exception as cerr:  #
                            if not display:
                                print(
                                    f"failed loading wlan0mon attempt #{tries}: {cerr!r}"
                                )
                    except Exception as err:  # from modprobe
                        if not display:
                            print("Failed reloading brcmfmac")
                        logging.error(
                            f"[Fix_Services] Failed reloading brcmfmac {err!r}"
                        )

                except Exception as nope:  # from modprobe -r
                    # fails if already unloaded, so probably fine
                    logging.error(f"[Fix_Services #{tries} modprobe -r] {nope!r}")
                    if not display:
                        print(f"[Fix_Services #{tries} modprobe -r] {nope!r}")
                    pass
            ...
            try:
                ...
                logging.error(f"[Fix_Services wifi.recon on] {err!r}")
                pwnagotchi.reboot()
