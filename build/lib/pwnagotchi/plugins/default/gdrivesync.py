...
class GdriveSync(plugins.Plugin):
    ...
    def on_loaded(self):
        ...
        if os.stat("/root/client_secrets.json").st_size == 0:
            logging.error(
                "[gDriveSync] /root/client_secrets.json is empty. Please RTFM!"
            )
            return
        ...
        try:
            ...
            if not self.backup:
                ...
                backup_folder = self.create_folder_if_not_exists(
                    self.options["backup_folder"]
                )

                # Continue with the rest of the code using backup_folder_id
                backup_folder_file_list = self.drive.ListFile(
                    {
                        "q": f"'{backup_folder}' in parents and mimeType = 'application/vnd.google-apps.folder' and trashed=false"
                    }
                ).GetList()
                if not backup_folder_file_list:
                    ...
                    if self.options["backupfiles"] is not None:
                        self.backupfiles = (
                            self.backupfiles + self.options["backupfiles"]
                        )
                ...
                zip_file_id = self.get_latest_backup_file_id(
                    self.options["backup_folder"]
                )
                if zip_file_id:
                    zip_file = self.drive.CreateFile({"id": zip_file_id})
                    zip_file.GetContentFile(
                    ...
                    with zipfile.ZipFile(
                        os.path.join(local_backup_path, "backup.zip"), "r"
                    ) as zip_ref:
                        zip_ref.extractall("/")
    ...
    def create_folder_if_not_exists(self, backup_folder_name):
        ...
        if backup_folder_id is None:
            ...
            logging.info(
                f"[gDriveSync] Created folder '{backup_folder_name}' with ID: {backup_folder_id}"
            )
    ...
    def upload_to_gdrive(self, backup_path, gdrive_folder):
        try:
            ...
            zip_file = self.drive.CreateFile(
                {"title": "backup.zip", "parents": [{"id": gdrive_folder}]}
            )
