...
class AutoBackup(plugins.Plugin):
    ...
    __license__ = "GPL3"
    __description__ = (
        "Backs up files when internet is available, with support for excludes."
    )
        ...
        if isinstance(interval, str):
            if interval.lower() == "daily":
                ...
                try:
                    ...
                    logging.error(
                        "AUTO-BACKUP: Invalid interval format. Defaulting to daily interval."
                    )
                    return 24 * 60 * 60
            ...
            logging.error(
                "AUTO-BACKUP: Unrecognized type for interval. Defaulting to daily interval."
            )
            return 24 * 60 * 60
    ...
    def on_internet_available(self, agent):
        ...
        if global_config is None:
            global_config = {}
        pwnagotchi_name = global_config.get("main", {}).get(
        ...
        try:
            ...
            for cmd in self.options["commands"]:
                formatted_cmd = cmd.format(
                    backup_file=backup_file, files=files_to_backup, excludes=excludes
                )
            ...
            logging.info(
                f"AUTO-BACKUP: Backup completed successfully. File created at {backup_file}"
            )
