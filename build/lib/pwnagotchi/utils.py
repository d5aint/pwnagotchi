...
def load_config(args):
    ...
    if not os.path.exists(args.config):
        ...
        if ref_defaults_data != defaults_data:
            print(
                f"!!! file in {args.config} is different than release defaults, overwriting !!!"
            )
            shutil.copy(ref_defaults_file, args.config)
    ...
    def load_toml_file(filename):
        ...
        if text.find("[main]") != -1:
            ...
            print(f"Converting dotted toml {filename}: {text[0:100]}")
            import toml
            ...
            try:
                ...
                print(f"Converted to new format. Original saved at {backup}")
            except Exception as e:
                print(f"Unable to convert {backup} to new format: {e}")
            return data
...
def iface_channels(ifname):
    ...
    output = subprocess.getoutput(
        f"/sbin/iw phy{phy} channels | grep ' MHz' | grep -v disabled | sed 's/^.*\\[//g' | sed s/\\].*\\$//g"
    )
