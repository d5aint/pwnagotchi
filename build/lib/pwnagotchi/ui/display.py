...
class Display(View):
    def __init__(self, config, state={}):
        ...
        self._render_thread_instance = threading.Thread(
            target=self._render_thread, daemon=True, name="Renderer"
        )
    ...
    def is_waveshare_any(self):
        return (
            self.is_waveshare_v1()
            or self.is_waveshare_v2()
            or self.is_waveshare_v4()
            or self.is_waveshare_v3
        )
    ...
    def image(self):
        ...
        if self._canvas is not None:
            img = (
                self._canvas
                if self._rotation == 0
                else self._canvas.rotate(-self._rotation, expand=True)
            )
        return img
