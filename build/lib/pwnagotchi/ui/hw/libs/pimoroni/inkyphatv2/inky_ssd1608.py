...
_RESOLUTION = {(250, 122): (136, 250, -90, 0, 6)}
...
class Inky:
    ...
    def __init__(
        ...
        self.cols, self.rows, self.rotation, self.offset_x, self.offset_y = _RESOLUTION[
            resolution
        ]
    ...
    def setup(self):
        ...
        if not self._gpio_setup:
            ...
            self._gpio.setup(
                self.busy_pin, self._gpio.IN, pull_up_down=self._gpio.PUD_OFF
            )
    ...
    def _update(self, buf_a, buf_b, busy_wait=True):
        ...
        self._send_command(
            ssd1608.DRIVER_CONTROL, [self.rows - 1, (self.rows - 1) >> 8, 0x00]
        )
    ...
    def set_image(self, image):
        ...
        self.buf = numpy.array(canvas, dtype=numpy.uint8).reshape(
            (self.cols, self.rows)
        )
