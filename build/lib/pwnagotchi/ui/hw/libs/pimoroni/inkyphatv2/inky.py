...
class Inky:
    ...
    def __init__(
        ...
        if self.eeprom is not None:
            if self.eeprom.width != self.width or self.eeprom.height != self.height:
                raise ValueError(
                    f"Supplied width/height do not match Inky: {self.eeprom.width}x{self.eeprom.height}"
                )
            if (
                self.eeprom.display_variant in (1, 6)
                and self.eeprom.get_color() == "red"
            ):
                self.lut = "red_ht"
    ...
    def set_image(self, image):
        ...
        if self.rotation % 180 == 0:
            self.buf = numpy.array(image, dtype=numpy.uint8).reshape(
