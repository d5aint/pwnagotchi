...
class I2COled(DisplayImpl):
    ...
    def layout(self):
        ...
        self._layout["width"] = (
            self._config["width"] if "width" in self._config else 128
        )
        self._layout["height"] = (
            self._config["height"] if "height" in self._config else 64
        )
    ...
    def initialize(self):
        ...
        logging.info(
            "Initializing SSD1306 based %dx%d I2C Oled Display on address 0x%X"
            % (width, height, i2caddr)
        )
        logging.info(
            "Available config options: ui.display.width, ui.display.height and ui.display.i2caddr"
        )
