...
class View(object):
    def __init__(self, config, impl, state=None):
        ...
        self._state = State(
            state={
                "channel": Components.LabeledValue(
                    color=BLACK,
                    label="CH",
                    value="00",
                ...
                "uptime": Components.LabeledValue(
                    color=BLACK,
                    label="UP",
                    value="00:00:00",
                ...
                "line1": Components.Line(self._layout["line1"], color=BLACK),
                "line2": Components.Line(self._layout["line2"], color=BLACK),
                "face": Components.Text(
                    value=faces.SLEEP,
                    position=(
                ...
                "shakes": Components.LabeledValue(
                    label="PWND ",
                    value="0 (00)",
                    color=BLACK,
        ...
        if config["ui"]["fps"] > 0.0:
            threading.Thread(
                target=self._refresh_handler, args=(), name="UI Handler", daemon=True
            ).start()
    ...
    def _refresh_handler(self):
        ...
        while True:
            try:
                ...
                logging.warning(f"non fatal error while updating view: {e}")
    ...
    def on_manual_mode(self, last_session):
        ...
        self.set(
            "face",
            (
                self._get_random_face(faces.SAD)
                if (last_session.epochs > 3 and last_session.handshakes == 0)
                else self._get_random_face(faces.HAPPY)
            ),
        )
    ...
    def on_new_peer(self, peer):
        ...
        # a good friend, positive expression
            ...
            face = self._get_random_face(
                random.choice((faces.MOTIVATED, faces.FRIEND, faces.HAPPY))
            )
        # normal friend, neutral-positive
        else:
            face = self._get_random_face(
                random.choice((faces.EXCITED, faces.HAPPY, faces.SMART))
            )
                ...
                if sleeping:
                    ...
                    if step % 2 == 0:
                        self.set(
                            "face",
                            (
                                self._get_random_face(faces.LOOK_R_HAPPY)
                                if good_mood
                                else self._get_random_face(faces.LOOK_R)
                            ),
                        )
                    else:
                        self.set(
                            "face",
                            (
                                self._get_random_face(faces.LOOK_L_HAPPY)
                                if good_mood
                                else self._get_random_face(faces.LOOK_L)
                            ),
                        )
