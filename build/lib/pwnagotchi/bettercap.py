...
class Client(object):
    ...
    async def start_websocket(self, consumer):
        ...
        while True:
            ...
            try:
                    ...
                    while True:
                        try:
                            ...
                            try:
                                ...
                                logging.warning(
                                    f"[bettercap] ping error - retrying connection in {sleep_time} sec"
                                )
                                await asyncio.sleep(sleep_time)
                                break
                ...
                logging.warning(f"[bettercap] retrying connection in {sleep_time} sec")
                await asyncio.sleep(sleep_time)
                continue
            except OSError:
    ...
    def run(self, command, verbose_errors=True):
        while True:
            try:
                r = requests.post(
                    f"{self.url}/session", auth=self.auth, json={"cmd": command}
                )
                ...
                logging.warning(f"[bettercap] retrying run in {sleep_time} sec")
                sleep(sleep_time)
            else:
                break
