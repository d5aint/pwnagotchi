...
class Peer(object):
    def __init__(self, obj):
        ...
        try:
            ...
            logging.warning(f"error while parsing peer timestamps: {e}")
    ...
    def update(self, new):
        if self.name() != new.name():
            logging.info(
                f"peer {self.full_name()} changed name: {self.name()} -> {new.name()}"
            )

        if self.session_id != new.session_id:
            logging.info(
                f"peer {self.full_name()} changed session id: {self.session_id} -> {new.session_id}"
            )
